...
function FavouriteTable({ movies, header, setOpen, tabKey }) {
  const [favouritesList, setFavouritesList] = useState(movies);
  const dispatch = useDispatch();

  useEffect(() => {
    setFavouritesList(movies);
  }, [tabKey, movies])
  ...
  function handleFavClick(mov) {
    dispatch(onUpdateFavourite(mov))
      .then(() => {
        ...
        const index = favouritesList.findIndex(fav => fav.imdbID === mov.imdbID)
        const updateFavs = [
          ...favouritesList.slice(0, index),
          newFavourite,
          ...favouritesList.slice(index + 1),
        ]
        setFavouritesList(updateFavs);
      });
  }
